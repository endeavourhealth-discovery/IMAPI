...
public class SetRepositoryImpl implements SetRepository {
    @Override
	public Set<TTEntity> getAllConceptSets(TTIriRef type) throws DALException {
		Set<TTEntity> result= new HashSet<>();
		StringJoiner sql = new StringJoiner("\n")
				.add("SELECT iri ")
				.add("FROM entity e")
				.add("JOIN entity_type et ON e.dbid = et.entity")
				.add("WHERE et.type = ? ");
		try (Connection conn = ConnectionPool.get();
             PreparedStatement getConceptSets = conn.prepareStatement(sql.toString())) {
            getConceptSets.setString(1, type.getIri());
		    ...
		    throw new DALException("Failed to get concept sets", e);
        }
		return result;
	}

	@Override
	public TTEntity getSetDefinition(String iri) throws DALException {
		...
		try (Connection conn = ConnectionPool.get();
		    ...
		    throw new DALException("Failed to get set definition", e);
        }
		return result;
	}

	@Override
	public Set<IMv2v1Map> getIMv2v1Maps(Set<EntitySummary> members) throws DALException {
	    ...
	    try(Connection conn = ConnectionPool.get();
	        ...
	        throw new DALException("Failed to get IMv2 -> IMv1 maps", e);
        }
	...
	public TTEntity getIM1Expansion(TTEntity conceptSet) throws DALException {
		...
		try (Connection conn = ConnectionPool.get();
		    ...
		    throw new DALException("Failed to get IMv1 expansion map", e);
        }
		return expanded;
	}
...
/*	public TTEntity getLegacyExpansion(String iri) throws SQLException {
	...
	public TTEntity getLegacyExpansion(TTEntity conceptSet) throws DALException {
        try (Connection conn = ConnectionPool.get();
            ...
            throw new DALException("Failed to expand legacy set", e);
        }
	}
	 ...
	 * @throws DALException with database access problems
	 */
	public TTEntity getExpansion(TTEntity conceptSet) throws DALException {
	...
	private void buildSuperClassSQL(TTIriRef superClass, int expressionNum, StringJoiner from, StringJoiner where) {
	 from.add("join entity focus_e"+expressionNum+" on focus_e"+expressionNum+"_tct.ancestor= focus_e"+expressionNum+".dbid");
		if (expressionNum==1)
			where.add("where");
		else
			where.add("and");
		where.add("focus_e"+expressionNum+".iri='"+superClass.getIri()+"'");
	}
