...
import org.endeavourhealth.imapi.model.sql.MySQLPropertyRangeWhere
...
/**
 * Phase 1 of the IMQ -> SQL pipeline: resolves an IMQ definition against the mapping in
 * IMQtoMYSQL.json (via [TableMap]) and produces a fully gap-filled [MySQLQuery] IR - every
 * table, alias, join key, select and where clause has already been decided here. The output
 * carries no SQL dialect syntax - [IRToJooqConverter] (phase 2) is a mechanical renderer of
 * this IR into jOOQ/MySQL.
 *
 * This is a full-coverage port of the decision logic in IMQtoSQLConverterKotlin, restructured
 * to stop at the IR instead of calling toSql(). It intentionally duplicates that logic rather
 * than modifying the production converter, so the existing /sql endpoint is unaffected while
 * this pipeline is validated. IMQtoSQLConverterKotlin remains the reference implementation for
 * any behavioural questions.
 *
 * One deliberate divergence: a match's own base table is only ever given an in-CTE alias when
 * path traversal requires disambiguation (matching the original). The CTE's own outer name
 * (used by later CTEs to join back to it) is a separate, always-assigned identifier - the two
 * are not interchangeable, exactly as in the original converter.
 */
class IMQPreparer(private val mapping: TableMap, private val queryRequest: QueryRequest) {

  private lateinit var queryTypeOfTable: Table
  ...
  data class PatientTraceIR(
    val withs: List<MySQLWith>,
    val queryIri: String,
    val patientId: String,
    val isPatientRooted: Boolean,
  )
  ...
  /** Cohort queries produce a single [MySQLQuery]; dataset (column-group) queries produce one per group. */
  fun prepare(query: Query): List<MySQLQuery> {
    if (query.typeOf == null || query.typeOf.iri == null) throw SQLConversionException("Query typeOf is null")
    queryTypeOfTable = getTableFromTypeAndProperty(query.typeOf.iri, null)
    resetState()

    val mySqlQuery = MySQLQuery()
    addMatchWithsRecursively(query, mySqlQuery)

    if (query.columnGroup != null) {
      val results = mutableListOf<MySQLQuery>()
      for ((index, columnGroup) in query.columnGroup.withIndex()) {
        resetState()
        val newMySqlQuery = MySQLQuery()
        if (columnGroup.name == null) columnGroup.name = "ColumnGroup$index"
        results.add(newMySqlQuery)
        if (query.`is` != null) newMySqlQuery.withs.add(getIsWith(query, newMySqlQuery))
        addMatchWithsRecursively(columnGroup, newMySqlQuery)
        if (columnGroup.and == null && columnGroup.or == null && columnGroup.`is` == null) {
        ...
        if (query.`return` == null) {
          ...
          // NB: the original embeds the bare IRI here unquoted, which isn't valid SQL syntax on
          // its own - quoting it via toSqlLiteral is a deliberate correctness fix, not a faithful
          // port of that spot.
          newMySqlQuery.selects.add(MySQLSelect(toSqlLiteral(query.iri), "query_result_id"))
          newMySqlQuery.selects.add(MySQLSelect("${lastCTE.alias}.$fk", "entity_id"))
          newMySqlQuery.selects.add(MySQLSelect(toSqlLiteral(columnGroup.name.replace(" ", "")), "column_group"))
    ...
    if (query.`return` == null) {
      ...
      mySqlQuery.selects.add(MySQLSelect(toSqlLiteral(query.iri), "query_result_id"))
    ...
    return listOf(mySqlQuery)
  }

  /** Produces the CTE chain plus the metadata needed to assemble the per-step patient-trace check rows. */
  fun preparePatientTrace(query: Query, patientId: String): PatientTraceIR {
    if (query.columnGroup != null) {
      throw SQLConversionException("Patient trace debugging is not supported for column-group (dataset) queries")
    }
    if (query.iri == null) throw SQLConversionException("Query iri is null")
    if (query.typeOf == null || query.typeOf.iri == null) throw SQLConversionException("Query typeOf is null")
    resetState()
    queryTypeOfTable = getTableFromTypeAndProperty(query.typeOf.iri, null)
    ...
    val mySqlQuery = MySQLQuery()
    addMatchWithsRecursively(query, mySqlQuery)
    if (mySqlQuery.withs.isEmpty()) throw SQLConversionException("Query produced no steps to trace")
    ...
    return PatientTraceIR(mySqlQuery.withs, query.iri, patientId, isPatientRooted)
  }
    ...
    return buildString {
      ...
      append(
        selects
          ...
          .joinToString(",\n") { select ->
            val rawAliasOrName = (select.alias ?: select.name)
            val key = "\"$rawAliasOrName\""
            val value = if (select.alias != null) rawAliasOrName else select.name
            " $key, $value"
          }
      )
      append("\n)")
      ...
      withJoins.add(
        MySQLJoin(
          ...
          wheres = if (isA.isExclude) mutableListOf(
            MySQLPropertyValueWhere("query_result_id", "=", toSqlLiteral(isA.iri), null, null),
  ...
  private fun addWhereConceptJoin(table: Table, fromField: String?, with: MySQLWith): Pair<MutableList<MySQLJoin>, String> {
    ...
    if (existingJoinForField != null) {
      return joins to (existingJoinForField.tableToAlias ?: bareConceptTableRef)
    }
    ...
    return joins to (alias ?: bareConceptTableRef)
  }
  ...
  private fun getTableFromTypeAndProperty(typeIri: String?, propertyIri: String?): Table {
    return mapping.getTableFromProperty(listOfNotNull(propertyIri))
      ?: mapping.getTableFromDataModel(typeIri)
  }

  private fun ensureUniqueAlias(baseAlias: String): String {
    fun normalize(a: String) = a.lowercase()
