...
public class TTNodeSerializerV2 extends StdSerializer<TTNode> {
  ...
  public void serialize(TTNode node, JsonGenerator gen, SerializerProvider prov) throws IOException {
    ...
    if (predicateTemplate == null) predicateTemplate = new ArrayList<>();
  ...
  private void serializeTemplatedPredicates(TTNode node, JsonGenerator gen, SerializerProvider prov) throws IOException {
    for (TTIriRef predicate : predicateTemplate) {
      if (node.get(predicate) != null) prov.defaultSerializeField(
        prefix(simpleProperties ? predicate.getIri().substring(predicate.getIri().indexOf("#")) : predicate.getIri()),
        node.get(predicate),
        gen
      );
    }
    ...
    if (predicates != null && !predicates.isEmpty()) {
      ...
      for (Map.Entry<TTIriRef, TTArray> entry : entries) {
        if (!predicateTemplate.contains(entry.getKey())) {
          prov.defaultSerializeField(
            prefix(simpleProperties ? entry.getKey().getIri().substring(entry.getKey().getIri().indexOf("#")) : entry.getKey().getIri()),
            entry.getValue(),
            gen
          );
        }
  ...
  public String prefix(String iri) {
    if (contextMap == null) return iri;
    else return contextMap.prefix(iri);
  }
