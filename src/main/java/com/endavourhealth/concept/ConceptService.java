...
//	ChildService childService;
	 ...
	 * @see ConceptService#getConceptDetail(String)
	 */
//	public Concept getConceptSummary(String iri) {
...
//	 * Retrieve the {@link Concept} with the given IRI. 
//	 * <br>
//	 * Note 1: The parent and children collections within the returned {@link Concept} will be initialised. 
//	 * Note 2: that if no {@link Concept} can be found with the given IRI then this method will return <b>null</b>
//	 * 
//	 * @param iri - the iri associated with the required concept
//	 * @return Concept - the concept model represented by the given iri. Note that this method will return null if the iri cannot be resolved
//	 */	
//	public Concept getConceptDetail(String iri) {
//		Concept concept = getConceptSummary(iri);
//		
//		com.endavourhealth.dataaccess.entity.Concept entity = conceptRepository.findByIri(iri);
//		treeService.addParents(concept, treeNodeBuilder);
//		
//		addChildren(concept);
//		addParents(concept);
//		
...
//	}
	...
	public <D> D fromConcept(String iri, FromConceptConverter<D> fromConceptConverter) {
		D d = null;
		
		com.endavourhealth.dataaccess.entity.Concept concept = conceptRepository.findByIri(iri);
		
		if(concept != null) {
			d = fromConceptConverter.convert(concept);
		}
		else {
			// TODO - error no concept found
		}
	...
	public <D> Set<TreeNode<D>> getParents(String iri, FromConceptConverter<D> fromConceptConverter) {
		Set<TreeNode<D>> parents = null;
		
		com.endavourhealth.dataaccess.entity.Concept concept = conceptRepository.findByIri(iri);
		
		if(concept != null) {
			parents = treeService.getParents(concept, fromConceptConverter);
		}
		else {
			// TODO - error no concept found
		}
	...
	public <D> Set<TreeNode<D>> getChildren(String iri, FromConceptConverter<D> fromConceptConverter) {
		Set<TreeNode<D>> children = null;
		
		com.endavourhealth.dataaccess.entity.Concept concept = conceptRepository.findByIri(iri);
		
		if(concept != null) {
			children = treeService.getChildren(concept, fromConceptConverter);
		}
		else {
			// TODO - error no concept found
		}
