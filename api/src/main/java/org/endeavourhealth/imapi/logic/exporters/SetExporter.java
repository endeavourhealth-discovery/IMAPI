            ...
            if (members != null && !members.isEmpty()) {
                ...
                if (entity.get(IM.DEFINITION)!=null)
                    result.addAll(setRepository.getSetExpansion(entity.get(IM.DEFINITION).asLiteral().objectValue(Query.class),
                        includeLegacy,new HashSet<>()));
                else
                  result.addAll(setRepository.getSetExpansion(new Query().setWhere(new Where().addFrom(TTAlias.iri(entity.getIri()).setIncludeSubtypes(true))),includeLegacy,new HashSet<>()));
            }
        }
