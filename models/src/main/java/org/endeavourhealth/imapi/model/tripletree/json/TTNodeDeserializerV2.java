    ...
    public TTNode deserialize(JsonParser jsonParser, DeserializationContext ctx) throws IOException {
        ...
        while (iterator.hasNext()) {
            ...
            if (!"@context".equals(key)) {
                ...
                if (value.isTextual())
                    result.set(iri(key), literal(value.asText()));
                else if (value.isArray())
                    result.set(iri(key), ctx.readValue(value.traverse(jsonParser.getCodec()), TTArray.class));
                else if (value.isObject()) {
                    if (value.has("@id"))
                        result.set(iri(key), iri(value.get("@id").asText()));
                    else
                        result.set(iri(key), ctx.readValue(value.traverse(jsonParser.getCodec()), TTNode.class));
                } else
                    result.set(iri(key), literal(value.asText()));
            }
        }
