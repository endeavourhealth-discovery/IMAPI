  ...
  void populateEntityFromJson(JsonNode node, TTEntityJava result) throws IOException {
    ...
    while (fields.hasNext()) {
      ...
      if ("iri".equals(key))
        ...
        if (value.isArray())
          result.set(TTIriRefExtensionsKt.iri(new TTIriRef(), helper.expand(key)), helper.getJsonNodeArrayAsValue(value));
        else
          result.set(TTIriRefExtensionsKt.iri(new TTIriRef(), helper.expand(key)), helper.getJsonNodeAsValue(value));
      }
    }
  }
