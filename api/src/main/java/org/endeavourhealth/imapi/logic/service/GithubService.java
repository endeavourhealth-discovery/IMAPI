...
public class GithubService {
  ...
  public GithubRelease getGithubLatestRelease() throws JsonProcessingException, ConfigException {
    GithubRelease test = configManager.getConfig(CONFIG.IMDIRECTORY_LATEST_RELEASE, new TypeReference<>() {
    });
    if (null == test)
      throw new ConfigException("Github config not found. This can be generated by hitting /api/github/updateGithubConfig", HttpStatus.BAD_REQUEST);
    return test;
  }
  ...
  public List<GithubRelease> getGithubReleases() throws JsonProcessingException {
    List<GithubRelease> test = configManager.getConfig(CONFIG.IMDIRECTORY_ALL_RELEASES, new TypeReference<>() {
    });
    if (null == test) throw new JsonParseException("EMPTYYYY");
    return test;
  }
