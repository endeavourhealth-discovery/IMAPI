...
public class PostgresRepository {
  ...
  public static DBEntry save(DBEntry entry) throws SQLException {
    String sql = """
      INSERT INTO
        query_queue(id, query_iri, query_name, query_request, user_id, user_name, queued_at, started_at, pid, finished_at, killed_at, status)
        VALUES (?, ?, ?, ?::jsonb, ?, ?, ?::timestamp, ?::timestamp, ?, ?::timestamp, ?::timestamp, ?)
        ON CONFLICT(id)
          DO UPDATE SET (query_iri, query_name, query_request, user_id, user_name, queued_at, started_at, pid, finished_at, killed_at, status) = (?, ?, ?::jsonb, ?, ?, ?::timestamp, ?::timestamp, ?, ?::timestamp, ?::timestamp, ?);
      """;
    try (PreparedStatement ps = PostgresConnectionManager.prepareStatement(sql)) {
      ...
      ps.setString(5, entry.getUserId());
      ...
      ps.setString(13, entry.getQueryIri());
      ps.setString(14, entry.getQueryName());
      ps.setString(15, om.writeValueAsString(entry.getQueryRequest()));
      ps.setString(16, entry.getUserId());
      ps.setString(17, entry.getUserName());
      ps.setTimestamp(18, Timestamp.valueOf(entry.getQueuedAt()));
      if (null != entry.getStartedAt()) ps.setTimestamp(19, Timestamp.valueOf(entry.getStartedAt()));
      else ps.setNull(19, java.sql.Types.VARCHAR);
      ps.setInt(20, entry.getPid());
      if (null != entry.getFinishedAt()) ps.setTimestamp(21, Timestamp.valueOf(entry.getFinishedAt()));
      else ps.setNull(21, java.sql.Types.VARCHAR);
      if (null != entry.getKilledAt()) ps.setTimestamp(22, Timestamp.valueOf(entry.getKilledAt()));
      else ps.setNull(22, java.sql.Types.VARCHAR);
      ps.setString(23, entry.getStatus().toString());
      ps.executeUpdate();
      return entry;
  ...
  public static Pageable<DBEntry> findAllByUserId(String userId, int page, int size) throws SQLException, JsonProcessingException {
