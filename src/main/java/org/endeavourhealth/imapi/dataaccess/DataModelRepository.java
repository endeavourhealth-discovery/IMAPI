...
public class DataModelRepository {
  ...
  public String checkPropertyType(String propIri) {
    try (IMDB conn = IMDB.getConnection()) {
      ...
      qry.setBinding("objProp", EnumUtils.asDbIri(ImVocab.DATAMODEL_OBJECTPROPERTY));
      qry.setBinding("dataProp", EnumUtils.asDbIri(ImVocab.DATAMODEL_DATAPROPERTY));
      try (TupleQueryResult rs = qry.evaluate()) {
        if (rs.hasNext()) {
          BindingSet bs = rs.next();
          if (bs.hasBinding("objectProperty")) return ImVocab.
            DATAMODEL_OBJECTPROPERTY.toString();
          else if (bs.hasBinding("dataProperty")) return ImVocab.
            DATAMODEL_DATAPROPERTY.toString();
        }
      }
    }
    return null;
