...
public class SetRepository {
	...
	public Set<TTEntity> getAllConceptSets(TTIriRef type) throws SQLException {
		...
		try (Connection conn = ConnectionPool.get()){
			assert conn != null;
			try (PreparedStatement getConceptSets = conn.prepareStatement(sql.toString())) {
	...
	public TTEntity getIM1Expansion(TTEntity conceptSet) throws SQLException {
		...
		try (Connection conn = ConnectionPool.get()){
			assert conn != null;
			try (PreparedStatement queryIM1Expansion = conn.prepareStatement(buildIM1ExpansionSQL(conceptSet))){
	...
	public TTEntity getLegacyExpansion(TTEntity conceptSet) throws SQLException {
		PreparedStatement queryLegacyExpansion;
		try (Connection conn = ConnectionPool.get()){
			assert conn != null;
