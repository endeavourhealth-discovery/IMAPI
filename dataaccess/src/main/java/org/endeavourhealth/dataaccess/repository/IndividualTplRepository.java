...
public class IndividualTplRepository {
	public TTNode findAllBySubjectIri(String iri) throws SQLException {
	    TTNode result = new TTNode();

        try (Connection conn = ConnectionPool.get()) {
            try (PreparedStatement stmt = conn.prepareStatement("SELECT p.iri AS p_iri, p.name AS p_name, o.iri AS o_iri, o.name AS o_name\n" +
                "FROM instance s\n" +
                "JOIN tpl_ins_object t ON t.subject = s.dbid\n" +
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        result.set(
                            ...
                            new TTIriRef(rs.getString("o_iri"), rs.getString("o_name"))
                        );
                    }
                }
            }
