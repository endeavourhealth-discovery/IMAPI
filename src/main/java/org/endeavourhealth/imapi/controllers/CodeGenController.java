...
public class CodeGenController {
  ...
  public HttpEntity<Object> generateCode(HttpServletRequest request,
                                         ...
                                         @Parameter(description = "The graphdb graph the data is stored under") @RequestParam(name = "graph", defaultValue = GRAPH.DISCOVERY) String graph) throws IOException {
    try (MetricsTimer t = MetricsHelper.recordTime("API.CodeGen.GenerateCode.GET")) {
      log.debug("GenerateCode");

      return codeGenService.generateCode(iri, templateName, namespace, graph);
    }
  }
  ...
  public String generateCodePreview(HttpServletRequest request,
                                    ...
                                    @Parameter(description = "The graphdb graph the data is stored under") @RequestParam(name = "graph", defaultValue = GRAPH.DISCOVERY) String graph
  ) throws IOException {
    try (MetricsTimer t = MetricsHelper.recordTime("API.CodeGen.GenerateCode.GET")) {
      log.debug("GenerateCodePreview");

      return codeGenService.generateCodeForModel(iri, template, namespace, graph);
    }
  }
}