...
---
## Input:
Given the queries as defined in the JSON files in the `Z:\Data\QOF`folder, which conform to the 
structure as defined in the `model/qof/QOFDocument` class, generate transformation
code that converts them into an IMQ format, as defined by the `model\imq\Query` class.
