...
public interface ConceptRepository extends JpaRepository<Concept, String> {
    ...
    @Query(value = "SELECT c.* FROM concept c WHERE c.name LIKE %:term% LIMIT 10", nativeQuery = true)
    List<Concept> search(@Param("term") String term);

    @Query(value = "SELECT c.* FROM concept c JOIN concept_tct tct ON tct.source = c.dbid AND tct.level > 0 JOIN concept t ON t.dbid = tct.target WHERE c.name LIKE %:term% AND t.iri = :root LIMIT 10", nativeQuery = true)
